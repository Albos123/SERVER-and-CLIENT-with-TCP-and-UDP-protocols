...
def threaded(connectionSocket, addr):
    while True:
        try:
            var0 = connectionSocket.recv(128)      #kety pranohen 128 bytes nga klienti
        except:
            break
        var0 = str(var0)
 ...
 #edhe pse nga ana e klientit e kam kufizuar qe mos te mirret pergjigjeja me e gjate se 128 bytes, me mire eshte qe ajo mos te dergohet fare nga serveri
        ...
        if(mesazhi!="GABIM"):
            connectionSocket.send(str.encode(mesazhi))
#nese rasisht ndodh ndonje gabim dhe serveri nuk mund t'a pranoje kerkesen nga klienti (shiko me larte komanda e pare tek try)
...
def Main():
    serverPort = 11000
    serverSocket = socket (AF_INET, SOCK_STREAM)    #krijimi i soketes ne protokollin TCP
    serverSocket.bind(('127.0.0.1', serverPort))    #lidhja e serverit per IP adresen e hostit dhe portin e caktuar
    serverSocket.listen(10)     #sa klient mund te presin ne linje
    while True:
        connectionSocket, addr = serverSocket.accept()      #connectionSocket paraqet IP adresen e hostit te serverit ndersa addr paraqet numrin e portit nga ana e klientit, porti ky i cili perdoret per handshake ne fillim dhe me pas lirohet ku komunikimi behet ne porte tjera te perzgjedhura vete nga protokolli
        start_new_thread(threaded, (connectionSocket,addr))     #krijimi i nje threadi, arsyeja pse ky thread i ka dy parametra eshte per shkak se connectionSocket perdoret per pranim dhe dergim te te dhenave ndersa addr nevojitet per metoden PORTNR
    serverSocket.close()

if __name__ == '__main__':
    Main()              #thirrja e metodes kryesore